"""
Módulo Centralizado de Limpeza e Enriquecimento de Dados
========================================================

Este módulo contém todas as funções de tratamento de dados usadas
pelos scripts de sincronização e correção.
...
def clean_usuario_nome(nome: str) -> Optional[str]:
    """
    Limpa nome de usuário removendo formatação GLPI.
    
    Entrada: "- da Silva Anderson (696)"
    Saída: "Anderson da Silva"
    """
    if not nome or not isinstance(nome, str):
        return None
    
    # 1. Remover ID entre parênteses no final
    nome_sem_id = re.sub(r'\s*\(\d+\)\s*$', '', nome).strip()
    
    # 2. Se começa com hífen, reordenar nome
...
def get_campo_name(campo_id) -> str:
    """
    Mapeia ID do campo para nome legível.
    Retorna nome ou 'Alteração de Sistema' se não encontrado.
    """
    if campo_id is None:
...
def treat_empty_value(val) -> Optional[str]:
    """
    Padroniza valores vazios para NULL.
    """
    if val is None:
        return None
...
def load_user_lookup(session) -> Dict[str, int]:
    """
    Carrega mapa de usuários para lookup reverso.
    Retorna: {'Nome Completo': ID}
    """
    try:
        ...
        mapping = {}
        for u in users:
            # Nome Completo: "Firstname Realname"
            parts = []
            if u.firstname:
                parts.append(u.firstname)
            if u.realname:
                parts.append(u.realname)
            
            full_name = " ".join(parts).strip()
            if full_name:
                mapping[full_name] = u.id
            
            # Também mapear pelo login
            if u.name:
                mapping[u.name] = u.id
