...
class TicketFollowup(Base):
    """Ticket followups/timeline (SIS schema)."""
    ...
    users_id = Column(Integer) # Author ID (GLPI)
    content = Column(Text)
    is_private = Column(Boolean, default=False)
    requesttypes_id = Column(Integer) # Source
    sincronizado_em = Column(TIMESTAMP(timezone=True), default=datetime.utcnow)

    def __repr__(self):
        return f"<TicketFollowup(id={self.id}, ticket={self.ticket_id})>"
