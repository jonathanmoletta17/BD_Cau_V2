...
import time
from src.services.ai import OllamaService # Import AI Service
...
def sync_tickets(client: GLPIClient, session, models: Dict, valid_ids: Dict, context: str = 'dtic', limit: int = None, since_date: datetime = None, sync_state_manager=None):
    ...
    while True:
        ...
        for t in tickets:
            ...
            TicketKnowledge = models.get('TicketKnowledge')
            
            # Decide if we run RAG:
            # 1. If Ticket is new or updated (we are inside the loop, so likely yes)
            # 2. But to save API calls, maybe check if TicketKnowledge exists?
            
            if TicketKnowledge:
                try:
                    # Check existing embedding
                    tk = session.query(TicketKnowledge).filter_by(ticket_id=ticket_obj.id).first()
                    
                    # Content for embedding
                    rag_content = f"{ticket_obj.titulo}\n{clean_html(ticket_obj.descricao)}"
                    
                    # If new or content changed significantly? 
                    # For now, let's sync if it's missing or if ticket was just updated (date_mod check)
                    # Simple rule: If we updated the ticket (ticket_obj was dirty), we assume we need to update embedding?
                    # Or simpler: if tk missing.
                    
                    should_embed = False
                    if not tk:
                        should_embed = True
                    # else:
                    #     # Optional: Check if content changed? 
                    #     pass 
                    
                    if should_embed:
                         # Initialize Service (Lazy load or global?)
                         # Better: Instantiate here or outside loop?
                         # Let's instantiate locally or assume stateless
                         ai_service = OllamaService()
                         
                         vector = ai_service.get_embedding(rag_content)
                         
                         if vector:
                             if not tk:
                                 tk = TicketKnowledge(ticket_id=ticket_obj.id)
                                 session.add(tk)
                             
                             tk.content = rag_content
                             tk.embedding = vector
                             tk.sincronizado_em = datetime.now()
                             
                         else:
                             # AI Service might be down, don't crash, just skip
                             pass
