...
import { fetchMockTicketDetail } from '../../constants';
...
export const TicketDetailModal: React.FC<TicketDetailModalProps> = ({ ticketId, isOpen, onClose }) => {
  ...
  // Fetch data when ticketId changes and modal is open
  useEffect(() => {
    if (isOpen && ticketId) {
      setLoading(true);
      ...
      // Simulate API Call
      fetchMockTicketDetail(ticketId)
        .then(res => {
          setData(res);
          setLoading(false);
        })
                       ...
                       return (
                         <div key={item.id} className="flex justify-center my-4">
                           <div className="bg-slate-800/80 text-slate-400 text-xs px-3 py-1 rounded-full border border-slate-700/50 text-center max-w-[80%]">
                              <span className="font-semibold text-slate-300">{item.author}</span>: {item.content}
                ...
                <div className="space-y-6">
                   ...
                   <div>
                      ...
                      <span className={`inline-block px-3 py-1 rounded text-xs font-bold border ${
                         data.priority === 'Alta' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 
                   ...
                   <a href="#" className="flex items-center justify-center gap-2 w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 rounded transition-colors text-sm font-medium">
